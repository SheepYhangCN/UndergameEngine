...
case 1060:
    ...
    for (i = -3; i < (((battle_board.left+32) + (battle_board.right+32)) + SPR_HEIGHT); i += SPR_HEIGHT)
    {
        ii += 1
		if(DURATION!=-1){
        b[ii] = Battle_MakeBoneV(((battle_board.x - (battle_board.left+32)) + i), (((battle_board.y + battle_board.down) + (LENGTH / 2)) + 5), LENGTH, 0, 0, COLOR, 0, 0, 0, ((DURATION + (PAUSE * 2)) + 24),OBJ,SPR)
		}else{
		b[ii] = Battle_MakeBoneV(((battle_board.x - (battle_board.left+32)) + i), (((battle_board.y + battle_board.down) + (LENGTH / 2)) + 5), LENGTH, 0, 0, COLOR, 0, 0, 0, DURATION,OBJ,SPR)
		}
        with (b[ii])
        {
...
case 1220:
	...
	instance_destroy(b3)
	instance_destroy(b4)
	Battle_SetBoardSize(64,64,40,40,240)
	Anim_Create(battle_board,"x",0,0,battle_board.x,56,240)
	break
case 1460:
	alarm[0]=-1
	break
...
case 1520:
	...
	break
case 1600:
	Battle_MakeBoneWallRight_FollowBoard(30,30,-1)
	//Battle_SetBoardSize(64,64,64,64,300)
	Anim_Create(battle_board,"x",0,0,battle_board.x,-56,300)
	break
case 1660:
	alarm[2]=1
	alarm[3]=30
...
case 1900:
	alarm[2]=-1
	alarm[3]=-1
	with(battle_bullet_bone_wall_follow_board){
	SPR_HEIGHT=sprite_get_width(spr_bone)+1
    ii = 0
...
case 2000:
	BlueSoulControl_ULBxNMD(DIR.DOWN)
	Battle_SetBoardSize(64,64,96,96,30)
	alarm[4]=31
	alarm[5]=60
	break
case 2440:
...
case 2910:
	...
	break
case 2960:
	Battle_SetBoardSize(64,64,240,-64-(240-160),30)
	BlueSoulControl_ULBxNMD(DIR.UP)
	p=Battle_MakePlatform(320-96-(240-160),320+24+battle_board.down)
	Anim_Create(p,"y",ANIM_TWEEN.BACK,ANIM_EASE.OUT,p.y,-battle_board.down,20)
	break
case 2975:
	BlueSoulControl_ULBxNMD(DIR.LEFT,12)
	break
case 2990:
	var PAUSE = 1
    var LENGTH = 30
	...
	var SPR_HEIGHT=sprite_get_width(SPR)+1
    var ii = 0
    for (i = -3; i < ((battle_board.left + 320) + SPR_HEIGHT); i += SPR_HEIGHT)
    {
        ii += 1
		if(DURATION!=-1){
    ...
    audio_play_sound(snd_exclamation, 0, false)
	#endregion
	Battle_MakeBoneWallLeft_FollowBoard(30,40,-1)
...
case 3020:
	...
	break
case 3030:
	Battle_SetBoardSize(64,64,240,240,30)
	break
case 3120:
	Battle_SetBoardSize(64,64,-72-(240-160),240,800)
	break
}
