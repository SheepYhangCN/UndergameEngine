...
if(_state==BATTLE_STATE.MENU){
		...
		if(Input_IsPressed(INPUT.CONFIRM)){
			...
			switch(_menu_choice_button){
				...
				case 2:
					...
					Anim_Create(battle_soul,"y",0,0,tempy,battle_board.y-battle_board.up-5+36+32*1+1-tempy,3)
					}else{
	...
	if(_menu==BATTLE_MENU.ACT_TARGET){
		...
		if(Input_IsPressed(INPUT.CONFIRM)){
			...
			Anim_Create(battle_soul,"y",0,0,tempy,battle_board.y-battle_board.up-5+36+32*1+1-tempy,2)
		}
	}else
